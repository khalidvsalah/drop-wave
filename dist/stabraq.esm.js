...
var Trigger = class {
  ...
  resize() {
    ...
    const bs = {
      ...
      yE: element2.offsetTop + element2.offsetHeight,
      x: element2.offsetLeft,
      xE: element2.offsetLeft + element2.offsetWidth
    };
    if (this.scroll) {
...
var Scroll = class extends Events {
  ...
  add(target, o) {
    ...
    o.dir = this.dir;
    const trigger = new trigger_default(target, o);
    this.loop();
    return trigger;
  }
  _raf(t) {
    ...
    this.speed.value = lerp(this.speed.value, this.speed.offset / this.speed.time, this.speed.ease);
    if (this.infinite) {
      if (rounded > this.dim) {
      ...
      this.infinite.map(([kid, bs]) => {
        ...
        if (rounded > this.dim - this.screen) {
          ...
          if (offsetS <= bs.z && offsetE >= bs.a)
            ...
            inRange(start, end, bs, kid, this.isY, rounded);
        } else
          inRange(start, end, bs, kid, this.isY, rounded);
      });
    } else
